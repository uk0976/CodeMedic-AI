...
class IssueItem(BaseModel):
    ...
    why_it_happens: str | None = Field(
        default=None, description="Root cause analysis of why this bug occurs."
    )
    ...
    fix_code: str | None = Field(
        default=None, description="Exact code snippet to replace the buggy lines."
    )
...
class SecurityItem(BaseModel):
    ...
    owasp_category: str | None = Field(
        default="A03:2021-Injection", description="OWASP Top 10 category."
    )
    ...
    remediation: str | None = Field(
        default=None, description="Concrete patch or secure code snippet."
    )
...
class CodeReviewItem(BaseModel):
    category: str = Field(
        description="Review area: SOLID, DRY, Naming, Type Hints, Docstrings, Error Handling."
    )
...
class ComplexityInfo(BaseModel):
    ...
    maintainability_index: int = Field(
        default=78, description="Maintainability index score out of 100."
    )
...
class AnalysisResponseSchema(BaseModel):
    ...
    code_explanation: str = Field(
        description="Detailed step-by-step AI explanation of code structure and flaws."
    )
    why_better: str = Field(
        description="Explanation of why the refactored code version is superior."
    )
    ...
    code_review: list[CodeReviewItem] = Field(
        default_factory=list, description="Senior code review evaluations."
    )
