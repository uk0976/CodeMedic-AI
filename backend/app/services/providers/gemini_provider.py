...
class GeminiProvider(BaseAIProvider):
    def __init__(self, api_key: str | None = None) -> None:
        try:
            ...
            raise ValueError(f"google-generativeai package is not installed: {e}") from e

        key = api_key or os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY")
        if not key:
            raise ValueError("GEMINI_API_KEY / GOOGLE_API_KEY is not configured.")
        self.genai.configure(api_key=key)  # type: ignore[attr-defined]
        self.model = self.genai.GenerativeModel("gemini-1.5-flash")  # type: ignore[attr-defined]
